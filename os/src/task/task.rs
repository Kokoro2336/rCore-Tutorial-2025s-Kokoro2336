...
use crate::fs::{File, Stdin, Stdout, OpenFlags};
...
impl TaskControlBlock {
    ...
    /// spawn a new process with the given path
    pub fn spawn(self: &Arc<Self>, path: String) -> isize {
        let parent_inner = &mut self.inner_exclusive_access();
        // create new memory set for the new process
        let elf_data = open_file(path.as_str(), OpenFlags::RDONLY)
                                .unwrap().read_all();

        let (memory_set, user_sp, entry_point) =
            MemorySet::from_elf(elf_data.as_slice());
        let trap_cx_ppn = memory_set
            .translate(VirtAddr::from(TRAP_CONTEXT_BASE).into())
            .unwrap()
            .ppn();
